...
def get_size(vid_name, path_prefix):
    vid_name = os.path.join(path_prefix, vid_name[:-4] + "_roi.mp4")
    cap = cv2.VideoCapture(vid_name)
    if not cap.isOpened(): 
        print("could not open :", vid_name, flush=True)
        return 0
...
class ByFrameCountSampler(Sampler):
    def __init__(self, dataset, max_frames_per_gpu, shuffle=True, seed=0):
        ...
        self.sizes = [get_size(item[0], dataset.path_prefix) for item in self.dataset.paths_counts_labels]
        print(self.sizes)
        self.max_frames_per_gpu = max(self.sizes)  # Defeats the purpose of Sampled; For now let it be.
        ...
        batch_indices = data_utils.batch_by_size(
            self._get_indices(), lambda i: self.sizes[i], max_tokens=self.max_frames_per_gpu
        )
